...
let private DisruptorChannelConsumer () = 
    let mutable ctr = 0L
    while true do
        let freeUpTo = ConsumerWait (seqRead._value, seqWrite)
        while ctr <= freeUpTo do
            ...
            if (ctr % 100L) = 0L then
                printfn "Async.Start asyncSend: %d" ctr
...
let DisruptorChannel (evnt:System.IO.Stream * FredisCmd) = 
    ...
    while prevSeqValToWaitOn <> seqWrite._value do  // ensure the earlier slots have been published before publishing this one
        Thread.SpinWait(nSpin)  
//                () // busy wait
    seqWrite._value <- writeSeqVal                  // publish, cant be written to by multiple threads because they are spin waiting on a different value of prevSeqValToWaitOn
    async {return ()} // this async is here to maintain type signature compatibility with DirectChannel
